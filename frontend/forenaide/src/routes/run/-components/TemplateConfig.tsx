...
export default function TemplateConfig({
	...
	const combobox = useCombobox("", (value: string) => {
		...
		const currentTemplate = currentTemplates.find((template) => template.id === value);
		...
		<div>
			{!configFile && (
				<div className="mb-2 mt-6">
					...
					<div>
            <Combobox options={currentTemplates.map((template) => ({label: template.name, value: template.id }))} comboboxState={combobox}/>
